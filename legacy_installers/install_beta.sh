...
LMOD_SUFFIX="beta"
CONFIG_CEES=config_cees/
#CONFIG_CEES=config_beta/
...
do
    ...
    do
        spack --config-scope=${CONFIG_CEES}/ --config-scope=config_env_${SPACK_ENV_NAME}/ install -j${CORECOUNT} ${pkg} %${compiler} target=${ARCH}
    done
    #
    # Parallel installs
    for mpi in ${mpis[@]}
    do
        ...
        #spack --config-scope=${CONFIG_CEES}/ install -j${CORECOUNT} $mpi %${compiler} target=${ARCH}
        #
        ...
        do
            echo "*** Package: $pkg"
            spack --config-scope=${CONFIG_CEES}/ --config-scope=config_env_${SPACK_ENV_NAME}/ install -j${CORECOUNT} $pkg  %${compiler} ^$mpi target=${ARCH}
        done
        #
        ...
        #do
        #    echo "*** Package: $pkg"
        #    spack --config-scope=${CONFIG_CEES}/ install -j${CORECOUNT} $pkg  %${compiler} ^$mpi target=${ARCH}
        #done
    #
        # problem children:
